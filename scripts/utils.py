...
def setup_resources(model_type, redownload=False):
	if not os.path.exists(f"/tmp/data"):
		os.mkdir(f"/tmp/data")
	start_time = time.time()

	metadata_filename = f'data/latest_index_metadata_{model_type}.json'
	...
	id_to_meta_mapping_filename = f'data/latest_index_id_to_meta_{model_type}.json'
	...
	index_filename = f'data/latest_index_{model_type}.dpr'
